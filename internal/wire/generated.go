...
func (v *Conversation) decode(b []byte, p string) error {
	...
	if out.HeadSeq < 0 {
		return badf(p+".head_seq", "head_seq must be >= 0, got %v", out.HeadSeq)
	}
	if out.HeadSeq > 9007199254740991 {
		return badf(p+".head_seq", "head_seq must be <= 9007199254740991, got %v", out.HeadSeq)
	}
...
func (v *DirectConversationRequest) decode(b []byte, p string) error {
	...
	var out DirectConversationRequest
	if s.Handle == nil {
