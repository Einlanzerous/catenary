...
- `schema/` — **the wire contract.** One schema, the generator over it, and 89 conformance vectors that TypeScript, Dart and Go all have to agree on, with a staleness guard that fails the build on a hand edit. Everything else here is downstream of this directory, and nothing in it is edited by hand.
...
The wire schema is the source of truth and the only place a message type is defined. TypeScript, Dart and Go are generated from it, 89 golden vectors are the contract between them, and a staleness guard fails the build the moment a generated file is hand-edited. Retrofitting this after two clients have drifted is far worse than paying for it up front, which is why R4 was a gate rather than a P1 task.
...
- **A ticket that needs more than one PR is split into sub-tasks first**, before the work starts — not delivered as a run of PRs all leading with the parent's key. Switchyard's hierarchy already carries it: a `subtask` takes a task/bug/spike parent. Two reasons beyond hygiene. Branch names lead with the key *so per-ticket spend attributes correctly*, and three branches named `CANT-18-*` bill three different pieces of work to one row. And decisions are written down at ticket boundaries as they happen — a ticket that quietly grows into three PRs has moved its own boundary, and the sub-task is what puts it back. On a Mode B ticket the split is proposed in the plan and approved with it, and **a sub-task of a Mode C ticket is filed `review_mode: full` explicitly** — a new ticket inherits the *project* default, which is `evidence`, never its parent's mode, so `CANT-63` and `CANT-67` would otherwise shed their line-by-line read at the moment they are split. Switchyard lets an agent lift a ticket into the human band and refuses the reverse with a 403, so this is enforceable rather than aspirational.
