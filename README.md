...
`GET /healthz` is dependency-free: if it answers, the process is alive. `GET /readyz` pings the database. Collapsing them would make a Postgres restart restart every Catenary instance, turning a recoverable dependency outage into a reconnect storm across every open socket.
