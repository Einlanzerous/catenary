...
5. **A live client learns a rename, a membership change and `muted` at its next catch-up, and not before.** The client may schedule a catch-up on its own policy to shorten that (CANT-35's call); obligation 2 makes any such catch-up correct. **Receipts are live, per instance, since CANT-107** (`Hub.read` broadcasts to every attached member session, origin included) — a receipt from another of the viewer's own devices on the same instance arrives without a catch-up, and a device attached to a different instance still learns of it at its next `/sync` (CANT-111).
...
- **The socket introduces a conversation via a `conversation` frame and its members' `user` frames on `ServerFrame`, emitted when the fan-out message is that conversation's first (`seq == 1`).** Decided by **CANT-103** (plan rev 3, ruling 0, picked E); built by **CANT-113** (the two frames) and **CANT-114** (the emission). A live client remains stale on conversation and user *changes* — a rename, a membership change, `muted` — until its next catch-up; that freshness half stays deferred, per CANT-103 ruling 2, to the ticket that writes the first such mutation.
