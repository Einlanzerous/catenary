...
**Five edges of that, settled by the reference client (CANT-126) so the other two do not each pick.** **The newest token is the chain's last proposal, which has never been presented**: presenting it means minting and persisting a new link first, so the chain grows by one link for every attempt that settles nothing. **Only Catenary's own 401 steps the walk back.** Any other non-answer, a 401 from a hop in front included, ends the attempt where it stands, and the next attempt starts from the newest token again. **A 200 collapses the chain** to the token it returned, at whichever link it arrived. **`present_proposal` moves the walk forward**: the proposal is presented next, with its own original proposal if it already has a link. **`fresh_proposal` is the one exception to reusing the original**: that link's proposal is replaced, every newer link is dropped, and the replacements are bounded — a server that says it to everything is an unknown outcome, not a loop.

**What bounds the chain (CANT-127).** **Persist `last_sent_at` with the chain: the device wall-clock time the newest link was written. Write it with the link, before the request leaves; clear it with the chain.** A credential is *unsettled* while its chain is non-empty. While unsettled, the client **refreshes on its own initiative only if a Catenary-authored response has reached that context later than `last_sent_at`** — a `/sync` 200 that decodes, Catenary's own 401, or a server frame that decodes; a hop's answer is none of them; a context with no response yet is closed; and *later than* is **strict**, so an answer at the same instant as the send is not one — **and no sooner than `last_sent_at + min(15 min, 5 s × 2^(n−1))`**, `n` being the chain's length. **The delay counts from the send.** Both halves of §1 wait for it. **A stamp that is missing — a chain written before this rule — or one in the future, from a clock set backwards, counts as absent, and absent reads the same way in both halves: the delay has elapsed, and any Catenary-authored response opens the gate. A context that has heard nothing yet is closed either way, so a chain with no stamp still waits for one answer rather than getting a free attempt.** A held refresh is not an error: the `/sync` that prompted it is not retried, and it is not logged per attempt. **There is no absolute bound**: every link is a request that may have been delivered, none can be re-sent or dropped without an answer, and the rules above bound how many are sent. **Nothing here changes which token is presented, with which proposal, or in what order, and nothing here adds a request.**
