...
// `membership_changed` has no producer yet — no mutation of an EXISTING conversation's
// membership (add, remove, promote-to-group) is written on `main` today; conversation
// creation's own membership insert already goes through `metadataBump`
// (`internal/store/metadata.go`), and CANT-91's guard forces the first such mutation
// through the same helper, which becomes `membership_changed`'s owner.
// `retention_purge` has no producer yet; see CANT-67. Promoted from an inline enum by
// CANT-74 so that every client-open enum is a named type in all three languages.
// CLIENT-OPEN (CANT-74): reachable from server root ServerFrame via ServerFrame >
...
// SOCKET IN WIRE VERSION 1 — a rename, a membership change or `muted` reaches a client
// only on its next `/sync`, and because the cursor moves only here, the next `/sync`
// always covers them. A receipt from another of the viewer's devices attached to THE
// SAME INSTANCE is a live exception, not deferred: `Hub.read` broadcasts a `receipt`
