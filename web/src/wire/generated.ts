...
const asToken = (v: unknown, p: string): Token => {
  const x = asStr(v, p)
  if (!TokenPattern.test(x)) bad(p, `Token must match ^[A-Za-z0-9_-]{43}$, got ${JSON.stringify(x)}`)
  return x
}
...
// `membership_changed` has no producer yet — no membership mutation is written on
// `main` today, and CANT-91's guard forces the first one through `metadataBump`
// (`internal/store/metadata.go`), which becomes its owner. `retention_purge` has no
// producer yet; see CANT-67. Promoted from an inline enum by CANT-74 so that every
// client-open enum is a named type in all three languages.
// CLIENT-OPEN (CANT-74): reachable from server root ServerFrame via ServerFrame >
...
export interface Conversation {
  ...
  firstUnreadSeq?: Seq
  // Newest seq the server holds for this conversation, or 0 when it holds none yet.
  // INLINE RATHER THAN Seq (CANT-75): Seq's own minimum is 1 because a MESSAGE's ordinal
  // is never 0, and that stays true — but head_seq is not a message's ordinal, it is a
  // fact about the conversation, and a conversation CAN hold none. Find-or-create can
  // hand back a direct conversation the instant it exists, before either member has sent
  // anything into it, and 0 is the honest answer for what a resync would count towards
  // there: nothing yet. What a resync counts towards, and what the numeric resync
  // progress is a fraction of.
  headSeq: number
...
export function decodeConversation(v: unknown, p = "Conversation"): Conversation {
  ...
  return {
    ...
    headSeq: o["head_seq"] === undefined || o["head_seq"] === null ? bad(`${p}.head_seq`, 'required field is missing') : ((v: unknown, p: string): number => { const x = asInt(v, p); if (x < 0) bad(p, `head_seq must be >= 0, got ${x}`); if (x > 9007199254740991) bad(p, `head_seq must be <= 9007199254740991, got ${x}`); return x })(o["head_seq"], `${p}.head_seq`),
...
// below it was received. THE SOCKET INTRODUCES A CONVERSATION AND ITS USERS ON THAT
// CONVERSATION'S FIRST MESSAGE, AND CARRIES NO FRESHNESS AFTER THAT — a rename, a
// membership change or `muted` on a conversation this session already holds reaches a
// client only on its next `/sync`, and because the cursor moves only here, the next
// `/sync` always covers them. A receipt from another of the viewer's devices is not
// among these: `Hub.read` broadcasts a `receipt` to every attached session of every
// member with no exclusion, the origin user's own other devices included, so it has
// been live over the socket since CANT-107 rather than deferred to `/sync`. `/sync` is
// the general path and the only one that carries conversations and users; in wire
// version 1 it is also the resume.
...
export function decodeEnrollResponse(v: unknown, p = "EnrollResponse"): EnrollResponse {
  ...
  return {
    userId: o["user_id"] === undefined || o["user_id"] === null ? bad(`${p}.user_id`, 'required field is missing') : asUuid(o["user_id"], `${p}.user_id`),
    deviceId: o["device_id"] === undefined || o["device_id"] === null ? bad(`${p}.device_id`, 'required field is missing') : asUuid(o["device_id"], `${p}.device_id`),
    accessToken: o["access_token"] === undefined || o["access_token"] === null ? bad(`${p}.access_token`, 'required field is missing') : asToken(o["access_token"], `${p}.access_token`),
    accessExpiresAt: o["access_expires_at"] === undefined || o["access_expires_at"] === null ? bad(`${p}.access_expires_at`, 'required field is missing') : asTimestamp(o["access_expires_at"], `${p}.access_expires_at`),
    ...
    refreshExpiresAt: o["refresh_expires_at"] === undefined || o["refresh_expires_at"] === null ? bad(`${p}.refresh_expires_at`, 'required field is missing') : asTimestamp(o["refresh_expires_at"], `${p}.refresh_expires_at`),
  }
}
...
export function encodeEnrollResponse(v: EnrollResponse): Record<string, unknown> {
  return compact({
    ...
    "access_expires_at": v.accessExpiresAt,
...
export function decodeRefreshResponse(v: unknown, p = "RefreshResponse"): RefreshResponse {
  const o = asObj(v, p)
  return {
    accessToken: o["access_token"] === undefined || o["access_token"] === null ? bad(`${p}.access_token`, 'required field is missing') : asToken(o["access_token"], `${p}.access_token`),
    accessExpiresAt: o["access_expires_at"] === undefined || o["access_expires_at"] === null ? bad(`${p}.access_expires_at`, 'required field is missing') : asTimestamp(o["access_expires_at"], `${p}.access_expires_at`),
...
export function encodeRefreshResponse(v: RefreshResponse): Record<string, unknown> {
  return compact({
    "access_token": v.accessToken,
    "access_expires_at": v.accessExpiresAt,
    "refresh_token": v.refreshToken,
...
// and `conversation_id` would duplicate the path — a body naming its own conversation
// id could disagree with the one it POSTed to, the same shape RefreshRequest already
// avoids by identifying the device through its token rather than a field beside it
// that could disagree with it. The path is the only source here, so there is nothing
// to disagree with. Same fields otherwise, the identical store insert, the identical
// Message back — CANT-75: the transport is the only thing that differs.
export interface MessageSendRequest {
  // The idempotency key, generated by the caller once per logical message and REUSED
  // verbatim on every retry — the same rule as ClientSend.client_id. The server
  // deduplicates on (account, client_id) and answers a duplicate with the original
  // Message, so a retry after an ambiguous failure is free and safe. A cron-driven bot
  // needs this MORE than a phone does: it retries mechanically, on a timer, possibly
  // from two replicas, and a client that mints a fresh key on retry has re-implemented
  // double-sending.
  clientId: Uuid
  text?: string
  attachments?: OutboundAttachment[]
  // Just the id — the server builds the whole ReplyRef from the live source message, the
  // same rule ClientSend.reply_to_message_id states.
  replyToMessageId?: Uuid
}

export function decodeMessageSendRequest(v: unknown, p = "MessageSendRequest"): MessageSendRequest {
  const o = asObj(v, p)
  return {
    clientId: o["client_id"] === undefined || o["client_id"] === null ? bad(`${p}.client_id`, 'required field is missing') : asUuid(o["client_id"], `${p}.client_id`),
    text: o["text"] === undefined || o["text"] === null ? undefined : asStr(o["text"], `${p}.text`),
...
export function encodeMessageSendRequest(v: MessageSendRequest): Record<string, unknown> {
  return compact({
    "client_id": v.clientId,
    "text": v.text === undefined ? undefined : v.text,
    "attachments": v.attachments === undefined ? undefined : v.attachments.map((x) => encodeOutboundAttachment(x)),
...
// `POST /conversations/direct` — find-or-create the direct conversation with a handle,
// so a bot can message someone it has never spoken to without already knowing a
// conversation id. Returns the same Conversation either way (CANT-75); the caller
// cannot tell from the response alone whether this call created it.
export interface DirectConversationRequest {
  // The other member's handle. Find-or-create: an existing direct conversation between
  // the caller and this handle is returned unchanged, and a pair that has never spoken
  // gets one conversations row and two conversation_members rows, atomically, so two
  // concurrent requests for the same pair cannot create two conversations.
  handle: string
}

export function decodeDirectConversationRequest(v: unknown, p = "DirectConversationRequest"): DirectConversationRequest {
  const o = asObj(v, p)
  return {
...
export function encodeDirectConversationRequest(v: DirectConversationRequest): Record<string, unknown> {
  return compact({
    "handle": v.handle,
  })
}
